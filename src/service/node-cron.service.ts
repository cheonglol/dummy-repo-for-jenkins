...
import logger, { LOGGING_TAGS } from "src/modules/logging";
...
class CronJobService {
  ...
  public registerTask(task: CronTask): void {
    ...
    logger.info(`Task "${task.name}" registered with schedule: ${scheduleKey}`, "CronJobService.registerTask", LOGGING_TAGS.SYSTEM);
  }
  ...
  public startCronJobs(): void {
    if (this.tasks.length === 0) {
      logger.info("No cron jobs registered.", `${CronJobService.name}.${this.startCronJobs.name}`, LOGGING_TAGS.STARTUP);
      return;
    }
    ...
    this.tasks.forEach((task) => {
      if (cron.validate(task.schedule)) {
        ...
        const scheduleKey = this.getScheduleKey(task.schedule);
        logger.info(`Cron job "${task.name}" started with schedule: ${scheduleKey}`, `${CronJobService.name}.${this.startCronJobs.name}`, LOGGING_TAGS.STARTUP);
      } else {
        logger.warn(`Invalid schedule format for task "${task.name}": ${task.schedule}`, `${CronJobService.name}.${this.startCronJobs.name}`, LOGGING_TAGS.STARTUP);
      }
    });
  }
  ...
  public stopAllCronJobs(): void {
    ...
    logger.info("All cron jobs stopped.", "CronJobService.stopAllCronJobs", LOGGING_TAGS.SYSTEM);
  }
}
